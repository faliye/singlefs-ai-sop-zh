...
实测（2026-09-13）：一条等后台实验跑完的循环用 `pgrep -f` 匹配二进制名，等了一整轮没有退出，最后靠写死的 pid 杀掉才停。
...
实测（2026-09-17，singlefs）：一个子 agent 写了 `cmd > log 2>&1; echo "exit=$?"`，那句 `echo` 不在重定向里、打到了标准输出，
它接着用 `until grep -q "^exit=" log; do sleep 10; done` 在日志里等这一行，这一行永远不会出现；它一直空转，到主 agent 查进度才被发现。
同一天另一个会话的一条测试跑了三个多小时没有任何输出，没人分得清是慢还是卡。
...
- 在 Claude Code 这类会话里，两次模型调用隔得太久，提示缓存会过期、整段上下文要重新写入：
  实测（同一天）一个子 agent 前台等编译与复跑、隔了五分钟以上才回来，一段活里重写了 6 次，每次 36 万到 59 万 token。
  长活放后台，定期回来复检一次，比一次干等几十分钟便宜。
...
实测（2026-09-19，这个仓，`time bash scripts/gate.sh`，机器 32 核）：整道门禁挂钟 26.7 秒，
其中 `selftest.sh` 一个人占 24.8 秒，而它的 CPU 只跑到 66%——一个核都没跑满，时间全花在逐个等子进程退出。
把它六批样本用例改成并行之后，350 例判定逐例不变，这一段降到 21.4 秒、CPU 105%。
同日下游 singlefs 的 `.claude/gate.d/59-crates-mutation-replay.sh` 逐条跑 `cargo test`，表里 173 条。
...
实测（2026-09-19，三个后台作业，第二个 `exit 7`）：
...
**脚本还没来得及打印诊断就被带走了**。实测：一个测试装置的五处失败分支，
`howto` 一句都没打出来，而装置说谎的时候最该看的就是那几句。
写条注释提醒拦不住这种事，所以它是 `scripts/shell-lint.sh` 里会红的一项。
