...
while IFS= read -r f; do
  ...
  if [[ ( "$f" == */kb/*.md || "$f" == */rules/*.md || "$base" == "CLAUDE.md" || "$f" == */agents/*.md || "$f" == */skills/*/SKILL.md ) && $WORDLIST -eq 1 ]]; then
    ...
    # 不带「见 / 如 / 照」的光秃形态（2026-09-17 在 singlefs 的 357 份 kb 上量：旧判据之外新命中 103 处，
    ...
    # 工作对象，不是文档位置（singlefs 的 agents/ 与 .claude/rules/ 上实测 5 处假红）。
    ...
    # 三类不判，各有理由（singlefs 的 343 份 kb 上量过）：
