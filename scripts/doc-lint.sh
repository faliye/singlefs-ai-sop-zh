...
PATTERNS=(
  ...
  # 「此前」这一族：原是使用者项目的一个本地阶段，判据通用，收归这里。
  # ⚠️ **只认「此前写的是 X」这一种形态**，不认「此前没有」「此前只量了两样」——
  # 后者陈述的是一个**至今仍然成立**的缺口，是论证的一部分，不是历史。
  # 实测（使用者项目）：不做这个区分时 30 处命中里只有 7 处是真的，假红压倒真红。
  '此前写的是'                   '正文不许写"此前写的是 X"，直接改成现行值'
  '此前写着'                     '正文不许写"此前写着 X"，直接改成现行值'
  '正文此前写'                   '正文不许回述自己此前写过什么，改成现行值'
  '本决策此前'                   '正文不许回述某条决策此前怎么写，改成现行值'
  '此前从未写'                   '"此前从未写"是历史陈述，写进「历史版本」'
  '此前没有写过'                 '"此前没有写过"是历史陈述，写进「历史版本」'
  '此前没写过'                   '"此前没写过"是历史陈述，写进「历史版本」'
  '此前不存在'                   '"此前不存在"是历史陈述，写进「历史版本」'
  '曾经写作'                     '"曾经写作"只能出现在「历史版本」节内'
  '曾经写着'                     '"曾经写着"只能出现在「历史版本」节内'
  '原先写的是'                   '正文不许写"原先写的是 X"，直接改成现行值'
...
STYLE=(
  ...
  '\(此乃\|实乃\|是故[^意]\|然则\|殊为\|不失为\)'  '古风腔：直接说这是什么、所以怎样'
...
while IFS= read -r f; do
  ...
  if [[ ( "$f" == */kb/*.md || "$f" == */rules/*.md || "$base" == "CLAUDE.md" || "$f" == */agents/*.md || "$f" == */skills/*/SKILL.md ) && $WORDLIST -eq 1 ]]; then
    ...
    # 不带「见 / 如 / 照」的光秃形态（2026-09-17 在使用者项目的 357 份 kb 上量：旧判据之外新命中 103 处，
    # 逐条看过全是指文档位置的真指代，零假红）。每一格的收窄都是量出来的：
    #   「上面 / 下面」有物理意义（「它上面的四个点」「多层树下面的单元」「标题下面一行」），
    #   所以左边只认句首、标点、粗体星号、表格竖线，或「以 把 按 取 在 是 了 成 进 含 从 为」，
    #   右边只认「的 / 是 / 数量词 / 第 / 保留 / 圈码或数字」——上面三个物理说法一个都不中；
    #   「下一节」会撞「下一节点」、「下一段」在文件系统里常指下一个段、「下一行」常指程序读的下一行，
    #   所以这三样只在「见」后面算，「上一行」另认「同上一行」「晚于 / 早于上一行」；
    #   「如下」后面必须紧跟标点，「例如下一次」不中；「上文」「下文」排掉「上下文」「上文件」。
    # 不查的三类也是量出来的：「前面 / 后面」56 处、「以上 / 以下」72 处，绝大多数是代码前后与数值阈值；
    # 「上方 / 下方」10 处，多半指带名字的位置（「字段表下方」「`min` 上方的注释」）。
    dlead='(^|[，。；：、！？（）()「」『』【】〔〕〈〉《》“”"*|[:space:]])'
    ctx="$ctx"'|[上下]述|[上下]表([^格头面达]|$)|上文([^件]|$)|(^|[^上])下文([^件]|$)'
    ctx="$ctx"'|见[上下]一(段|行|节)|同上一行|[早晚]于[上下]一行|如下[：:，。；—]'
    # 「见上方 / 见下方」：带了「见」就只能是文档位置，物理意义的「上方 / 下方」不带它
    # （「字段表下方」「`min` 上方的注释」）——所以这一格判，光秃的「上方 / 下方」仍不判。
    # 「下一节」与「上一节」对称收窄：排掉「下一节点」「下一节拍」。
    ctx="$ctx"'|见[上下]方|下一节([^点拍]|$)'
    ...
    # kb 里「本决策」指的是被检索出来的那一条，是真自指；规范文本（规则、agent 定义）
    # 讲的是怎么处理一条决策、一个实验，「这个实验的每个量」「这条决策管什么」指的是
    # 工作对象，不是文档位置（使用者项目的 agents/ 与 .claude/rules/ 上实测 5 处假红）。
    # 所以规范文本只判指文档结构的那几个词，领域对象词留给 kb。
    if [[ "$dockind" == "kb 正文" ]]; then
      self="$lead本$tails|$lead该$gtails|$lead这(条|个)(决策|实验|不变量)"
    else
      self="$lead本(章节|文档|条([^件目]|$)|节([^点]|$)|表([^格]|$))"
    fi
    # 指被测系统而不是文档位置的，一律排除
    ...
    if [[ "$dockind" == "kb 正文" ]]; then
      if rounds="$(printf '%s\n' "$scan" | awk '
        ...
        while IFS= read -r r; do
          rln="${r%%$'\t'*}"; rtx="${r#*$'\t'}"
          bad "$rel:$rln  kb 正文里的「本轮」锚不到具体哪一轮"
          say "        > $(printf '%s' "$rtx" | cut -c1-80)"
          howto "把指代换成锚：写明是哪一天的那一轮，或者点名那次实验的编号。" \
                "确实泛指任何一轮的规则句，改写成不带指代的说法（「跑它的那一轮」→「产出这份输出的那次运行」）。" \
                "判据是检索：这一条被单独端出来时，读的人能不能知道是哪一轮。"
          filefail=1
        done <<< "$rounds"
      fi
    fi
