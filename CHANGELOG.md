...
修回归（`scripts/gate.sh`）：
- 0.0.55 加的「agent 定义纪律」阶段调用了几十行之后才定义的 `run_rules_lint`，
  而 SOP 仓自己没有 `.claude/agents/`，那个 `if` 从来没进去过——三仓门禁全绿，
  装了 agent 定义的项目一跑就断在那一行，只跑了 121 项、没有汇总行、后面的阶段一个都没跑。
  那一段本来就是重复的（「规则纪律（项目本地）」已经把 agent 定义当额外文件扫了），删掉；
  顺带把它的条件放宽成「有 `.claude/rules` 或 `.claude/agents` 就跑」，补上只有 agents 的项目那个缺口。
- **新增收尾断言**：跑完必须打印汇总行，中途退出就在 stderr 明说「门禁没跑完就退出了」并返回原退出码。
  中途退出时「没跑」和「跑过了」在输出里长得一模一样，这一条以前谁也没守。

去名（`rules/rules-discipline.md` 第 8 条的彻底执行）：
- 脚本注释里的实测出处 28 处、门禁样本 4 处、`README.md`、`install.sh`、`templates/` 全部改成不指名。
- 唯一一处**代码里**的下游名字：git ref `refs/singlefs/gate-ok` 改成 `refs/sop/gate-ok`。

使用者项目的本地阶段收归上游，这一版十二条：
- `doc-lint.sh`：历史语气「此前写的是 X」这一族 12 条模式（每条 why 各不相同，样本逐条盯着）。
- `shell-lint.sh` 新增 **S7**：pipefail 下以提前退出的 `grep -q` 收尾的管道——命中会被读成没命中。
  上游 `lib.sh` 自己有两处这种写法，一并改成先落变量再判。
- 新脚本 `script-modes.sh`（脚本执行位在暂存区里没丢）：**上游自己的 changelog-lint、show-me-test、
  version-discipline 三个 `.sh` 的执行位正丢在暂存区里**，这一版一并修好。
- 新脚本 `stage-selftest.sh`（项目本地阶段自己会不会红）：上游的「门禁判别力」只覆盖共享脚本，
  项目在 `.claude/gate.d/` 里接的那一批此前没人验。
- 新脚本 `link-targets.py`（相对链接与「第 N 节」指向）：两类失效都是静默的。
- 新脚本 `history-ordinal.sh`（本次新增的历史条目撞没撞号）：公共编号先到先得。
- 新脚本 `hooks-registered.sh`（工具层的闸注册着、自检过得了）：钩子被删掉或改坏时那道闸静默消失。
- 新脚本 `relay-timing-lint.py`（读子进程输出的循环不许一边打时间戳一边转打）：射程与冻结证据清单都改成现算。
- 新脚本 `number-name-sync.sh`（编号与简称在 doc-lint 够不到的源码注释与记录里也要一致）。
- `rules-lint.sh` 的词法说明加「计划第 N 节」；`gate.sh` 新增 `run_stage_may_skip`：退出码 77 记「本次未跑」。
- 下游对应的 10 个本地阶段与 2 个库脚本随之删除，判据一条没丢——每条都在使用者项目上实测过。

自检从 373 涨到 395 个用例。

## 0.0.55 — 2026-09-21

**0.0.54 把指代检查扩到规范文本时，连工作对象一起判了；顺带收编 singlefs 的时间指代检查。**

规则（`rules/`）：
- `kb-discipline.md` 第 1 条新增**时间指代**：「本轮」「这一轮」「上一轮」「前一轮」「上轮」一律不许，
  写成锚（日期加「那一轮」，或点名那次实验的编号）。门禁的判据是这一行或它所属的最近一个标题里有没有日期。
  三类不查：「下一轮」是泛指；「本次」「上次」「这次」多挂在技术对象上；历史节的每条都挂在日期标题下。
- `kb-discipline.md` 第 1 条的「上方」「下方」从「门禁不查」改成**只查带「见」的两个形态**
  （「见上方」「见下方」）：带了「见」就只能是文档位置，光秃的那两个多指物理位置。
  同时补上光秃的「下一节」，与「上一节」对称收窄（排掉「下一节点」「下一节拍」）。
- `rules-discipline.md` 第 7 条写明规范文本与 kb 的判据差别：自称只判指文档结构的那几个词
  （「本节」「本章节」「本表」「本文档」「本条」）；「这个实验」「这条决策」这类领域对象词不判，
  规范文本讲的就是怎么处理这些对象；时间指代也不判，规则对每一轮都生效。
- `rules-discipline.md` 新增第 8 条「正文不写某一个使用者的东西」：规范是给任何使用者读的，
  正文里不许出现使用它的项目的名字、路径、文件名，也不许拿它的决策号、实验号当示例。
  使用者名单登记在 `I18N` 的 `consumers=`。按这一条回扫了三语：规则里指名的 3 处改写成不指名的说法，
  拿下游决策号当示例的 5 处换成「编号（简称）」这样的形态，skill 正文的 description 里的项目名换成「本项目」。

脚本（`scripts/`）：
- `doc-lint.sh` 的自指词表按文件类型分成两套。0.0.54 把 kb 那套原样用在规范文本上，
  在 singlefs 的 `.claude/agents/` 与 `.claude/rules/` 上误判 5 处（「这个实验的每个量」
  「这条决策管什么、不管什么」——那是工作对象，不是文档位置）。
- `doc-lint.sh` 的上下文指代补两格：「见上方」「见下方」与光秃的「下一节」。
  这两格原先只有 singlefs 的本地阶段 `20-kb-shape.sh` 判，上游漏着。
- **时间指代检查收归上游**：原是 singlefs 的本地阶段 `.claude/gate.d/25-kb-deictic.sh`，
  判据通用（管的是文档怎么写，不依赖文件系统），挪进 `doc-lint.sh` 的 D 段，只对 `kb/*.md`。
  左边界排掉「样本轮」这类构词——在 singlefs 的门禁样本上实测撞到一处。
- `rules-lint.sh` 新增第八条判据：规则正文出现使用者项目的名字就判红。名单从**被扫那个仓**的 `I18N`
  读 `consumers=`，本包名（`family=`）先整串挖掉——上游脚本不写死下游的名字；项目根没有 `I18N`，
  所以项目扫自己的规则时这一条无对象可判，成功行里如实报出来。
  新样本 `fixtures/rules-lint/consumer`（红）与 `consumerok`（绿：包名里含着使用者的名字，不算）。
- 新样本 `fixtures/doc-lint/kbround`（红：三处「本轮」锚不到日期）与 `kbroundok`
  （绿：标题带日期、同行带日期、「下一轮」「本次」都放行）。
  `selfok` 样本那行补了日期锚——它测的是「本工程 / 本轮 / 本机」不算自指，新检查会判它。

怎么确认这三条会红：新检查在 singlefs 的 443 份文件上跑过，指代类的红全部消失、零假红；
时间指代那条与它的前身 `25-kb-deictic.sh` 在同一批 kb 上对拍，差异只剩那个脚本自己的红样本。
自检 371 个用例全过。

## 0.0.54 — 2026-09-21

**证据的冻结只管本轮；没铺满全仓的改动要逐份登记；规范文本的正文不许指位置。**

规则（`rules/`）：
- `evidence-discipline.md`「原样保存的证据不许事后改」之下新增三段：冻结只在本轮内有效，
  这一轮提交之后由下一次提交删掉上一次留下的那批，本次的留存、本次的验证照旧做得了，
  要查删掉的去版本库历史里找；旧证据是参考不是依据，对早先的结论有疑问就重新验证、不翻旧证据，
  要推翻按推论纪律重走一轮；正文引产物的检查按「这次改动」判、不按全仓判。
- `show-me-test.md` 新增一节「一条改动不同步铺满全仓时，排除的每一份都要登记」：
  规范、操作、改动只铺一部分文件时，没铺到的逐条登记进排除表、`#` 后写为什么，
  **没有排除表就意味着全仓都要改**；判据是「改了这句话就成假的，或者改了某道闸就失效」；
  排除表指向不存在的路径判红，且要能被一条命令读。

- `rules-discipline.md` 新增第 7 条「正文不许用位置指代，也不许自称」：射程是 `rules/*.md`、`CLAUDE.md`、
  `agents/*.md`、`skills/*/SKILL.md`，判据与 `kb/*.md` 同一套，逐词射程以 `kb-discipline.md` 第 1 条为准；
  位置写成名字——小节写成它的标题，表写成它判的那件事，别处的事实链到它所在的文件；
  带 `<!-- doc-lint:rule-definition -->` 牌的规则文件，只有反引号与「」里的举例不判，正文照判。
- `show-me-test.md` 末句由「门禁的质量就是这个项目的上限」改为
  「门禁有多强，这个项目的下限就有多高」：同一篇里推论第 3 条写的是「门禁是下限，不是上限」，
  末句又拿同一个词指另一个量（门禁质量决定项目保证得了什么）。英文那份最直白：
  `a floor, not a ceiling` 对上 `this project's ceiling`。
- 三语规则按第 7 条回扫：`CLAUDE.md`、`command-safety.md`、`engineering-philosophy.md`、
  `evidence-discipline.md`、`kb-discipline.md`、`machine-first.md`、`show-me-test.md`、`sop-first.md`、
  `writing-discipline.md`、`skills/gate/SKILL.md` 里的「上一节」「下面那张表」「上面第一句」「同上」
  全部换成名字。门禁词表够不着的形态（「第二句」「上一段」「这张表」「这份规则」）一并人工改掉。

脚本（`scripts/`）：
- `naming-lint.sh` 的缩写词表删掉 `sb superblock`：缩写是为它对应的那个概念设的，概念没了就删行，
  不改指到别的全称——那等于声称这个缩写是别的词的缩写。
- `doc-lint.sh` 的指代检查（上下文指代、自指称呼）射程从 `kb/*.md` 扩到规范文本：
  `rules/*.md`、`CLAUDE.md`、`agents/*.md`、`skills/*/SKILL.md`。扫的正文换成挖掉举例的那一份，
  列举指代词的那几张表不会自己判自己；失败信息按文件类型分成「kb 正文」与「规范正文」。
  新增样本 `fixtures/doc-lint/rulesref`（红：`rules/` 与 `CLAUDE.md` 共 6 处位置指代都要报出来）
  与 `rulesrefok`（绿：带牌的规则文件里，反引号与「」中的举例不判）。

三语一致（`I18N`）：
- 日文 `show-me-test.md` 补回中英都有而它缺掉的那一段（证据对所有人是同一把尺子，提交的人自己就能先量一遍）。
- 英文与日文 `CLAUDE.md` 的接法表补上 agents 那一行——中文有，那两份一直缺。

## 0.0.53 — 2026-09-20

**规则文件只写怎么做：立第四类文档的纪律、立会红的检查、把三语规则里的历史与原因清出去。**

规则（`rules/`）：
- 新增 `rules-discipline.md`：管 `rules/*.md` 与项目的 `.claude/rules/*.md`。正文只写四样（怎么做、做什么不做什么、推荐怎么做、什么规则在哪里）；判据留下、论证搬走；搬出正文的实测、论证与定案记述直接删掉，不另立落点——共享规则的历史照旧进 `CHANGELOG.md`，一版一节；指向历史的链接要带劝阻句「除非要查来历，别读它」，一字不许省；规则文件不留历史节。
- `writing-discipline.md` 的三类文档表第三行改指向它；`design-doc-discipline.md` 里「`CLAUDE.md` 与 `rules/*.md` 不留历史节」那句的权威记录搬进新规则。
- 全部 14 份规则、`CLAUDE.md`、`agents/INDEX.md`、`skills/crash-test` 与 `skills/decide` 按新规则回扫：实测、论证小节、定案日期、门禁判别力的经过从正文删掉，正文只留怎么做。三语章节结构逐份对齐。

门禁（`scripts/`，三仓同一份）：
- 新增 `rules-lint.sh`：判七条——记录小节、论证小节、带日期的行、解释性段落、解释性半句、词法说明、没带劝阻句的历史链接。豁免四种：「」里的日期、反引号里的阈值日期、表格行（不判后三条）、排除清单登记的文件。判据的词表是中文的，别的语言仓显式报未实现（退出码 77），不假装通过。
- `gate.sh` 两个阶段：「规则纪律」扫本包的 `rules/`，「规则纪律（项目本地）」扫项目的 `.claude/rules/`。两个阶段都把 `CLAUDE.md`、`agents/*.md`、`skills/*/SKILL.md` 一起扫——它们和规则一样是照着执行的。项目不需要自己在 `.claude/gate.d/` 里接。
- `selftest.sh`：新增 15 个 rules-lint 判别力样本（每个红样本只犯一处，绿样本踩在四种豁免的边界上）；`gate.sh` 的桩与 want 补上新阶段。
